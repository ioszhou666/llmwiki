...
from .extractors import dump_json, extract_add_field_instruction, extract_fix_actions, extract_replace_instruction
...
class AnswerEngine:
    ...
    def apply_fixes(self, rel_path: str) -> dict[str, str]:
        ...
        self.audit.write("document_fixed", source=rel_path, target=f"output/fixed/{source_path.name}")
    ...
    def _apply_docx_actions(self, source_path: Path, target_path: Path, actions: list[object]) -> None:
        ...
        if document_xml is not None:
            ...
            for field in dict.fromkeys(add_fields):
                insert = "<w:p><w:r><w:t>" + f"补充字段: {field}" + "</w:t></w:r></w:p>"
    ...
    def _apply_pptx_actions(self, source_path: Path, target_path: Path, actions: list[object]) -> None:
        ...
        for name, payload in list(files.items()):
            ...
            add_fields = [action.field_name for action in actions if getattr(action, "kind", "") == "add_field" and action.field_name]
            for field in dict.fromkeys(add_fields):
                marker = '<p:sp><p:txBody><a:p><a:r><a:t>' + f"补充字段: {field}" + "</a:t></a:r></a:p></p:txBody></p:sp>"
    ...
    def _build_claude_context(self, title: str, parsed: object) -> dict[str, object]:
        ...
        for rel_path in merged_paths[:8]:
            ...
            snippets.append({"path": rel_path, "content_preview": content[:500], "comments": comments[:5]})
        return {
            "question": title,
    ...
    def _select_keyword_document(self, keyword: str, suffixes: set[str]) -> str | None:
        ...
        for candidate in candidates:
            ...
            for rel_path in self.index.list_document_paths():
                ...
                if rel_parent in parent_dirs and not self.policy.is_path_denied(rel_path) and Path(rel_path).suffix.lower() in suffixes:
                    return rel_path
        return None
